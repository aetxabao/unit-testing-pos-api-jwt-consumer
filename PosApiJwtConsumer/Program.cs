...
namespace PosApiJwtConsumer
{
    public class Consumer
    {
        ...
        public static List<Message> GetMessages(string token)
        {
            ...
            Console.WriteLine("\nGetMessages:");
            Console.WriteLine(response.Content);
            if (response.Content == "Invalid customer" || response.Content == "No message was found" || response.Content.Trim().Length == 0)
            {
        ...
        {
            var client = new RestClient(BASEURL);
            var request = new RestRequest($"/Messages/{message}", Method.PUT);
            request.AddHeader("Authorization", "Bearer " + token);
            request.AddParameter("message", message);
